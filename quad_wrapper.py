...
def run_quadtree_lsh_query(df, ranges, text_col, top_n, num_perm, bands, min_shingles):
    ...
    for i, attr in enumerate(selected_attrs):
        ...
        min_n = (raw_min - scaler.mins[i]) / scaler.ranges[i]
        max_n = (raw_max - scaler.mins[i]) / scaler.ranges[i]
        min_norm.append(min_n)
        max_norm.append(max_n)

    centers = tuple((mx + mn) / 2 for mx, mn in zip(max_norm, min_norm))
    half_sizes = tuple((mx - mn) / 2 for mx, mn in zip(max_norm, min_norm))
    ...
    if matched_count > 1:
        ...
        df_filtered = pd.DataFrame(lsh_data).set_index("id")
        
        lsh_index, _, skipped = build_lsh_index(
            df=df_filtered, text_col=text_col, 
